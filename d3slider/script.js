...
var circle, circle2, fig, fig2, figh, figw, h, marker_pos, my_slider, my_slider2, slider2_callback, slider2_g, slider_callback, slider_g, slider_height, slider_margin, slider_width, svg, svg2, w, xscale;
...
fig2.insert("line").attr("x1", 0).attr("x2", figw).attr("y1", figh / 2).attr("y2", figh / 2).attr("stroke", "slateblue").attr("stroke-width", 2);

circle2 = fig2.insert("circle").attr("id", "circle").attr("cx", Math.random() * figw).attr("cy", figh / 2).attr("r", 10).attr("fill", "slateblue");

slider2_g = svg2.insert("g").attr("transform", "translate(0," + figh + ")");

slider2_callback = function(sl) {
  return circle2.attr("cx", xscale(sl.value()));
};
